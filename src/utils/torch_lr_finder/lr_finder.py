...
class DataLoaderIter(object):
    ...
    def inputs_labels_from_batch(self, batch_data):
        ...
        index, video_start, (inputs, labels) = batch_data

        return index, video_start, (inputs, labels)
...
class TrainDataLoaderIter(DataLoaderIter):
    ...
    def __next__(self):
        try:
            batch = next(self._iterator)
            index, video_start, (inputs, labels) = self.inputs_labels_from_batch(batch)
...
class LRFinder(object):
    ...
    def _train_batch(self, train_iter, accumulation_steps, non_blocking_transfer=True):
        ...
        for i in range(accumulation_steps):
            index, video_start, (inputs, labels) = next(train_iter)
    ...
    def _validate(self, val_iter, non_blocking_transfer=True):
        ...
        self.model.eval()
        with torch.no_grad():
            for index, video_start, (inputs, labels) in val_iter:
                if torch.any(video_start):
                    self.model.reset()

...
class GridTrainer:
    ...
    def __init__(self, config: dict, train: bool = True, batch_size=None, load_from_checkpoint: bool = True, seed=0):
        ...
        self.time_logger = time_logger.TimeLogger(restart_time=60 * 60 * 1.19)
    ...
    def load_after_restart(self):
        """
        checks if a previous checkpoint and previous metric log exists and loads it if possible.
        """
        try:
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.get_starting_parameters(what="epoch"), self.config["num_epochs"])):
            ...
            # save current status and start evaluation every "evaluation_steps" epochs
            self.logger["loss"].append(int(self.logger["running_loss"]) / len(self.dataset))
    ...
    def eval(self, random_start=True, eval_length=29 * 4, save_file_path=None, load_most_recent=True):
        ...
        with torch.no_grad():
            ...
            running_loss = 0
            video_freq = 20  # every 20 epochs a video with the results will be saved
            flickering = 0
            flickering_sum = 0
            ...
            flickering_sum2 = 0
            fip = 0
            to_PIL = T.ToPILImage()  # conversion of tensor to PIL image
            metrics = defaultdict(AverageMeter)  # to save the results

            # Ensure model is in evaluation status its states are newly initiated
            self.model.eval()
            ...
            for i, batch in enumerate(loader):
                ...
                if self.logger["epochs"][-1] % video_freq == 0 or self.logger["epochs"][-1] == self.config[
                    "num_epochs"] - 1:
                    for j in range(self.batch_size):  # if batch size > 1 assert that the video writing works
                        ...
                        # break after certain amount of frames
                if i == eval_length:
                    break
